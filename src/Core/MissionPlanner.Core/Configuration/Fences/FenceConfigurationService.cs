    ...
    ILogger<FenceConfigurationService> logger) : IFenceConfigurationService
{
    ...
    private FenceOperationReport Failure(VehicleId vehicleId, string message, FenceValidationResult? validation = null) =>
        new(false, message, GetSnapshot(vehicleId), validation ?? FenceValidationResult.Valid);
    ...
    private static FencePlan Freeze(FencePlan plan) => new(
        plan.ReturnPoint,
    ...
    private sealed class Workspace
    {
        ...
        public FenceConfigurationSnapshot Snapshot(VehicleId vehicleId) => new(
            vehicleId,
            Freeze(LocalPlan),
