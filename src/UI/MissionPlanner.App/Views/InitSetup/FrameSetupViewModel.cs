...
public sealed partial class FrameSetupViewModel : SetupWorkflowDetailViewModel
{
    ...
    [RelayCommand]
    private Task LoadCommandAsync() => LoadAsync();
...
public sealed partial class FrameParameterSettingViewModel : ObservableObject
{
    ...
    public FrameParameterChange ToChange() => new(Name, Setting.CurrentValue, SelectedOption!.Value, Setting.ParameterType);

    private string OptionLabel(float value) =>
        Options.FirstOrDefault(option => Math.Abs(option.Value - value) <= 0.0001f) is { } option
            ? $"{option.Label} ({value})"
...
public sealed partial class FrameRecommendationViewModel : ObservableObject
{
    ...
    public FrameRecommendationViewModel(FrameInitialParameterRecommendation recommendation) => Recommendation = recommendation;
    ...
    /// <returns>The reviewed parameter change.</returns>
    public FrameParameterChange ToChange() => new(
        Recommendation.Name,
