...
public sealed class FirmwareArtifactDownloaderTests
{
    ...
    public async Task OversizedDownloadStopsAndDeletesPartialArtifact()
    {
        var bytes = ValidPackage();
        var store = new MemoryStore();
        var downloader = CreateDownloader(new StaticHandler(bytes), store, bytes.Length - 1);
    ...
    {
        return new FirmwareArtifactDownloader(new HttpClient(handler), store, new ApjFirmwarePackageReader(Options.Create(new FirmwareOptions())),
            Options.Create(new FirmwareOptions { MaximumArtifactBytes = maximumBytes }), TimeProvider.System);
    }

    private static FirmwareArtifact Artifact(byte[] bytes)
    {
    ...
    private static byte[] ValidPackage()
    {
        return File.ReadAllBytes(Path.Combine(AppContext.BaseDirectory, "Fixtures", "valid.apj"));
    }
    ...
    private sealed class NoLengthHandler(byte[] bytes) : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK) { Content = new NoLengthContent(bytes) });
        }
    ...
    {
        public void Report(FirmwareProgress value)
        {
            report(value);
        }
    }
    ...
    private sealed class MemoryStore : IFirmwareArtifactStore
    {
        ...
        {
            return Task.FromResult<IFirmwareStoredArtifact?>(stored?.Metadata.CacheKey == cacheKey ? stored : null);
        }

        public Task<IFirmwareArtifactWriter> CreateTemporaryAsync(string cacheKey, CancellationToken cancellationToken = default)
        {
            return Task.FromResult<IFirmwareArtifactWriter>(new Writer(this));
        }
