    ...
    TimeProvider timeProvider) : IDfuHexArtifactDownloader
{
    ...
    public async Task<DfuArtifact> DownloadAsync(Uri sourceUri, string platform, int? boardId, CancellationToken cancellationToken = default)
    {
        ...
        if (!IsTrustedHttps(sourceUri))
        {
            throw new DfuArtifactResolutionException("Official DFU artifacts require a configured trusted HTTPS host.");
        }
        ...
        if (await store.TryGetAsync(cacheKey, cancellationToken).ConfigureAwait(false) is { } cached)
        {
            try
            {
                if (!IsTrustedHttps(cached.Metadata.SourceUri))
                {
                    throw new DfuArtifactResolutionException("The cached artifact source is no longer trusted.");
                }
        ...
        try
        {
            ...
            if (!IsTrustedHttps(finalUri))
            {
                throw new DfuArtifactResolutionException("The official artifact redirected to an untrusted host.");
            }
            ...
            while ((read = await source.ReadAsync(buffer, cancellationToken).ConfigureAwait(false)) > 0)
            {
                ...
                if (total > maximum)
                {
                    throw new DfuArtifactResolutionException("The official Intel HEX artifact exceeded the configured size limit while streaming.");
                }
    ...
    private bool IsTrustedHttps(Uri uri)
    {
        return uri.IsAbsoluteUri && uri.Scheme == Uri.UriSchemeHttps && string.IsNullOrEmpty(uri.UserInfo) &&
