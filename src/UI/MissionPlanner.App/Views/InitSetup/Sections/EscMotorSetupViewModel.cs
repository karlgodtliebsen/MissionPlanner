...
public sealed partial class EscMotorSetupViewModel : SetupWorkflowDetailViewModel
{
    ...
    /// <inheritdoc />
    public override void Cancel() => _ = actuatorService.EmergencyStopAsync();
    ...
    private bool CanTest() => SupportsMotorTest && activeVehicle.IsOnline && TestState != MotorTestState.Running;
    ...
    private bool CanStop() => TestState == MotorTestState.Running;

    [RelayCommand(CanExecute = nameof(CanStop))]
    private Task StopAsync() => actuatorService.EmergencyStopAsync();

    private async Task<bool> ConfirmSafetyAsync() => await confirmation.ConfirmAsync(
    ...
    private void OnStateChanged(object? sender, MotorTestStateChangedEventArgs args) => dispatcher.Dispatch(() => Show(args.Snapshot));
