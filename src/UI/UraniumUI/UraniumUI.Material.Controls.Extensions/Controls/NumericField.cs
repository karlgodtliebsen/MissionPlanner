...
public partial class NumericField : TextField
{
    ...
    {
        if (updatingText || string.Equals(args.OldTextValue, args.NewTextValue, StringComparison.Ordinal)) return;
        ...
        SetValidity(inRange);
        if (!inRange || Value.Equals(parsed)) return;
    ...
    private void OnCompleted(object? sender, EventArgs args) => Commit();
    private void OnUnfocused(object? sender, FocusEventArgs args) => Commit();
    ...
    private void Commit()
    {
        ...
        if (ClampOnCommit) parsed = Math.Clamp(parsed, Min, Max);
        else if (parsed < Min || parsed > Max) { SetValidity(false); return; }

        if (!Value.Equals(parsed)) Value = parsed;
        FormatValue();
    }
    ...
    {
        if (!updatingValue && !EntryView.IsFocused) FormatValue();
        SetValidity(true);
        if (!oldValue.Equals(newValue)) ValueChanged?.Invoke(this, new(oldValue, newValue));
    }

    internal void OnRangeChanged()
    {
        if (correctingRange) return;
        correctingRange = true;
        try
        {
            if (Min > Max) Max = Min;
            var corrected = CoerceValue(Value);
            if (!Value.Equals(corrected)) Value = corrected;
            else FormatValueWhenNotEditing();
        }
    ...
    {
        if (!EntryView.IsFocused) FormatValue();
    }
    ...
    private void FormatValue()
    {
        ...
        string formatted;
        try { formatted = Value.ToString(format, ResolveCulture()); }
        catch (FormatException) { formatted = Value.ToString("G15", ResolveCulture()); }
        if (string.Equals(Text, formatted, StringComparison.Ordinal)) { SetValidity(true); return; }
    ...
    {
        if (string.IsNullOrWhiteSpace(text)) { value = default; return false; }
        var style = NumberStyles.AllowDecimalPoint;
        if (AllowSign) style |= NumberStyles.AllowLeadingSign;
    ...
    private bool IsPotentialNumber(string? text)
    {
        ...
        if (remaining.Contains(number.NegativeSign, StringComparison.Ordinal) ||
            remaining.Contains(number.PositiveSign, StringComparison.Ordinal)) return false;
        if (Count(remaining, number.NumberDecimalSeparator) > 1) return false;
    ...
    {
        if (string.IsNullOrWhiteSpace(CultureName)) return CultureInfo.CurrentCulture;
    ...
    {
        if (IsTextValid != valid) SetValue(IsTextValidPropertyKey, valid);
    }
