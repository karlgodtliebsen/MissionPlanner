...
public sealed class ParameterEditSessionFactory : IParameterEditSessionFactory, IDisposable
{
    ...
    private void OnSessionChanged(object? sender, EventArgs args) => Changed?.Invoke(this, EventArgs.Empty);
}
...
internal sealed class ParameterEditSession : IParameterEditSession, IDisposable
{
    ...
    public bool TrySetPending(string name, double value, out string? error)
    {
        ...
        lock (sync)
        {
            ...
            updated = field with
            {
                ...
                WriteStatus = NearlyEqual(value, field.LiveValue) ? ParameterEditWriteStatus.Unchanged : ParameterEditWriteStatus.Pending,
                WriteMessage = null
            };
            fields[name] = updated;
        }
        ...
        lock (sync)
        {
            ...
            fields[parameter.Name] = field with
            {
                ...
                WriteStatus = remainsModified
                    ...
                    : field.WriteStatus == ParameterEditWriteStatus.Applying ? ParameterEditWriteStatus.Applying : ParameterEditWriteStatus.Unchanged,
    ...
    private static string? ValidateTypeRange(MavParamType type, double value) => type switch
    {
    ...
    private static ParameterFieldMetadata ProjectMetadata(ParameterMetadata? metadata)
    {
        if (metadata is null)
        {
            return ParameterFieldMetadata.Empty;
        }

        return new ParameterFieldMetadata(
