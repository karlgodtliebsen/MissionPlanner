...
public sealed record FirmwareArtifact
{
        ...
        long? imageSize = null)
    {
        ...
        if (imageSize is <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(imageSize), "Decoded firmware image size must be positive.");
        }
    ...
    /// <summary>Gets the decoded application image size reported by the manifest, when available.</summary>
