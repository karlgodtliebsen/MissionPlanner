...
public sealed partial class TelemetryLogsTabViewModel
{
    ...
    public ObservableRangeCollection<TelemetryStoredLog> Logs { get; } = new();
    /// <summary>One bounded page of decoded packets.</summary>
    public ObservableRangeCollection<TelemetryPacketRow> Packets { get; } = new();
    /// <summary>Selected stored recording.</summary>
    [ObservableProperty]
    public partial TelemetryStoredLog? SelectedLog { get; set; }
    ...
    [ObservableProperty]
    public partial bool FollowReplay { get; set; }
    ...
    [RelayCommand]
    private Task RefreshLogsAsync() => LibraryOperationAsync(async token =>
    {
    ...
    [RelayCommand]
    private Task OpenStoredAsync() => LibraryOperationAsync(async token =>
    {
    ...
    private async Task OpenPacketsAsync(TelemetryStoredLog item, CancellationToken token)
    {
        ...
        try
        {
            ...
            packetStream = stream;
            packetIndex = index;
            var updated = item with { Metadata = item.Metadata with { Started = index.StartedAt, Ended = index.EndedAt } };
    ...
    [RelayCommand]
    private Task ReplayStoredAsync() => LibraryOperationAsync(async token =>
    {
    ...
    [RelayCommand]
    private Task StopReplayAsync() => LibraryOperationAsync(async token =>
    {
    ...
    [RelayCommand]
    private Task ExportLogAsync() => LibraryOperationAsync(async token =>
    {
        if (SelectedLog is { } item)
        {
    ...
    [RelayCommand]
    private Task DeleteLogAsync() => LibraryOperationAsync(async token =>
    {
        if (SelectedLog is not { } item)
        {
            return;
    ...
    [RelayCommand]
    private void CancelLogOperation() => operationCancellation?.Cancel();

    [RelayCommand]
    private Task FilterPacketsAsync() => LibraryOperationAsync(token => ReadPacketPageAsync(0, token));

    [RelayCommand]
    private Task NextPacketsAsync() => LibraryOperationAsync(token => ReadPacketPageAsync(nextPacket, token));

    [RelayCommand]
    private Task PreviousPacketsAsync() => LibraryOperationAsync(token => ReadPacketPageAsync(Math.Max(0, pageStart - 200), token));

    [RelayCommand]
    private Task JumpPacketsAsync() => LibraryOperationAsync(token =>
    {
        ...
        if (!DateTimeOffset.TryParse(JumpTimestamp, out var timestamp))
        {
            throw new ArgumentException("Enter a UTC date/time, for example 2026-09-19T10:30:00Z.");
        }

        return ReadPacketPageAsync(TelemetryPacketBrowser.FindIndex(packetIndex, timestamp), token);
    });
    ...
    private async Task ReadPacketPageAsync(int start, CancellationToken token)
    {
        ...
        byte? ParseByte(string text) => string.IsNullOrWhiteSpace(text) ? null : byte.Parse(text);
