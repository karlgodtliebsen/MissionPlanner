...
public sealed class LogsNavigationTests
{
    ...
    public async Task DefaultsToTelemetryAndRestoresSelectionAcrossHosts()
    {
        ...
        Control Create(int section) => section == 1 ? application : telemetry;
        using (var model = new LogsViewModel(Create, state, NullLogger<LogsViewModel>.Instance, dispatcher, events))
        {
        ...
        using var reopened = new LogsViewModel(Create, state, NullLogger<LogsViewModel>.Instance, dispatcher, events);
