...
public partial class TransponderTabViewModel : ObservableObject, IDisposable
{
    ...
    {
        this.activeVehicle = activeVehicle; this.registry = registry; this.dispatcher = dispatcher;
        registry.Changed += OnChanged; activeVehicle.Changed += OnChanged; Refresh();
    }
    /// <summary>Gets discovered transponders.</summary>
    public ObservableCollection<TransponderComponentState> Components { get; } = [];
    /// <summary>Gets bounded nearby traffic.</summary>
    public ObservableCollection<AdsbTrafficTrack> Traffic { get; } = [];
    /// <summary>Gets the explicit support/discovery state.</summary>
    [ObservableProperty] public partial string Status { get; private set; } = "No transponder discovered";
    /// <inheritdoc />
    public void Dispose() { registry.Changed -= OnChanged; activeVehicle.Changed -= OnChanged; }
    private void OnChanged(object? sender, EventArgs args) => dispatcher.Dispatch(Refresh);
    private void Refresh()
    {
        var systemId = activeVehicle.VehicleId?.SystemId;
        if (systemId is null) { Status = "No active vehicle"; return; }
