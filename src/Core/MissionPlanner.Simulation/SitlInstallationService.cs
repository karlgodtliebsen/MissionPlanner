    ...
    ILogger<SitlInstallationService> logger) : ISitlInstallationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return !capability.CanExecuteNative || release.Platform != capability.Platform ||
            ...
            : packageManager.PrepareAsync(release, progress, cancellationToken);
    }
        ...
        IReadOnlyList<SitlInstallation> installations)
    {
        ...
        return selected is null
            ? new SitlInstallationResolution(
                SitlInstallationState.Missing,
                null,
                $"Pinned SITL version '{profile.Binary.Version}' is not installed for {profile.FirmwareFamily}.")
            : new SitlInstallationResolution(selected.State, selected, selected.Message);
    }
