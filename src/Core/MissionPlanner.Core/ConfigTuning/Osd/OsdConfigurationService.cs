    ...
    ILogger<OsdConfigurationService> logger) : IOsdConfigurationService
{
        ...
        int row)
    {
        ...
        {
            workspace.Session.TrySetPending(item.ColumnParameterName, oldColumn, out var _);
            workspace.Session.TrySetPending(item.RowParameterName, oldRow, out var _);
    ...
    {
        return GetScreen(workspace, screenNumber).Items.Select(item => new OsdPreviewItem(
            item.Key,
    ...
    private static IReadOnlyList<string> ScreenParameterNames(OsdScreenDefinition screen)
    {
        return screen.ScreenParameterNames
    ...
    {
        return workspace.Screens.FirstOrDefault(screen => screen.Number == screenNumber)
    ...
    {
        return value?.Contains("dynamic", StringComparison.OrdinalIgnoreCase) == true ||
    ...
    {
        return System.Globalization.CultureInfo.InvariantCulture.TextInfo.ToTitleCase(key.Replace('_', ' ').ToLowerInvariant());
    }
    ...
    private static OsdImportResult Failure(string error)
    {
        return new OsdImportResult(false, 0, [], [], [error]);
    }
