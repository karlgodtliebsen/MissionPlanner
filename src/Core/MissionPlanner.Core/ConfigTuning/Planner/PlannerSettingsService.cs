...
public sealed class PlannerSettingsService : IPlannerSettingsService
{
    ...
    /// <inheritdoc />
    public ValueTask<PlannerSettingsSaveResult> SaveStartup(PlannerSettings settings, bool isTutorialVisibleAtStartup, CancellationToken cancellationToken = default)
    {
        settings.Appearance = settings.Appearance with
        {
    ...
    {
        return SaveAsync(new PlannerSettings(), cancellationToken);
    }
    ...
    private static (PlannerSettings Settings, bool Migrated) ParseAndMigrate(string document)
    {
        ...
        if (settings.SchemaVersion is <= 0 or > PlannerSettings.CurrentSchemaVersion)
        {
