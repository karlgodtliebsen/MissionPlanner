...
public sealed class PlannerActiveMapSourceStore(IPlannerSettingsService settings) : IActiveMapSourceStore
{
    ...
    {
        var result = await settings.SaveAsync(settings.Current with
        {
            Map = settings.Current.Map with
            {
        ...
        if (!result.Success)
        {
            throw new InvalidOperationException("The fallback map source could not be persisted.");
        }
    }
}
