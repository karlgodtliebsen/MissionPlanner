...
public partial class IntroductionImageView : UserControl, IDisposable
{
    ...
    {
        if (disposed || !IsLoaded || DataContext is not IntroductionImage image || string.IsNullOrWhiteSpace(image.Path))
        {
            return;
        }
    ...
    private async Task LoadImageAsync(string path, CancellationTokenSource cancellation, int version)
    {
        try
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(() =>
            {
                ...
                {
                    ScreenshotImage.Source = new Bitmap(new MemoryStream(bytes));
                }
            });
        }
    ...
    {
        return !disposed && IsLoaded && !cancellation.IsCancellationRequested && version == loadVersion &&
