...
public sealed class BatteryConfigurationService : IBatteryConfigurationService
{
    ...
    public async Task<BatteryApplyResult> SetValueAsync(VehicleId vehicleId, int instance, BatterySetting setting, double value, CancellationToken cancellationToken = default)
    {
        ...
        return await WriteAndConfirmAsync(vehicleId, name, (float)value, parameter.Type, cancellationToken).ConfigureAwait(false)
            ? new BatteryApplyResult(true, $"Confirmed {name} by vehicle readback.", reboot)
            : new BatteryApplyResult(false, $"Readback did not confirm {name}. Reconnect, refresh, and verify before flying.");
    }
    ...
    private async Task<BatteryApplyResult> CalibrateAsync(VehicleId vehicleId, int instance, BatterySetting setting, double measured, double reference, string quantity, CancellationToken cancellationToken)
    {
        ...
        return await WriteAndConfirmAsync(vehicleId, name, (float)corrected, parameter.Type, cancellationToken).ConfigureAwait(false)
            ? new BatteryApplyResult(true, $"Confirmed {quantity} calibration: {name} set to {corrected:0.####}.")
            : new BatteryApplyResult(false, $"Readback did not confirm the {quantity} calibration.");
    }
    ...
    private static string? Reject(BatterySetting setting, double value, int instance, IReadOnlyDictionary<string, VehicleParameter> values)
    {
        ...
        {
            BatterySetting.Capacity or BatterySetting.VoltageMultiplier or BatterySetting.CurrentPerVolt when value <= 0 => $"{setting} must be greater than zero.",
            BatterySetting.LowVoltage when value > 0 && ReadValue(values, instance, BatterySetting.CriticalVoltage) is { } critical && critical > 0 && value <= critical => $"Low voltage ({value:0.##} V) must be above the critical voltage ({critical:0.##} V).",
            BatterySetting.CriticalVoltage when value > 0 && ReadValue(values, instance, BatterySetting.LowVoltage) is { } low && low > 0 && value >= low => $"Critical voltage ({value:0.##} V) must be below the low voltage ({low:0.##} V).",
            BatterySetting.LowCapacity when value > 0 && ReadValue(values, instance, BatterySetting.CriticalCapacity) is { } critical && critical > 0 && value <= critical => $"Low capacity ({value:0} mAh) must be above the critical capacity ({critical:0} mAh).",
            BatterySetting.CriticalCapacity when value > 0 && ReadValue(values, instance, BatterySetting.LowCapacity) is { } low && low > 0 && value >= low => $"Critical capacity ({value:0} mAh) must be below the low capacity ({low:0} mAh).",
            var _ => null
        };
    }
    ...
    private BatteryLiveReading ReadLive(VehicleState state, int index)
    {
        ...
        return index == 2 && state.Power.SecondaryBattery is { } secondary
            ? new BatteryLiveReading(secondary.VoltageVolts, secondary.CurrentAmps, secondary.ConsumedMah,
                secondary.RemainingPercent, secondary.IsStale(now, staleWindow), true)
            : new BatteryLiveReading(null, null, null, null, true, false);
    }
