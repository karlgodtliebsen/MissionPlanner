...
public sealed class MotorLayoutResolver
{
    ...
    public MotorLayout? Resolve(IReadOnlyDictionary<string, VehicleParameter> parameters)
    {
        ...
        if (!parameters.TryGetValue(className,out var frameClass)) return null;
        var value=(int)Math.Round(frameClass.Value); var count=value switch { 1=>4, 2=>6, 3=>8, 4=>8, 5=>6, 7=>3, 12=>12, 13=>10, _=>0 };
        ...
        return new(value,type,$"{ClassName(value)} / type {type}",motors);
    }
    private static string ClassName(int value)=>value switch {1=>"Quad",2=>"Hexa",3=>"Octa",4=>"OctaQuad",5=>"Y6",7=>"Tri",12=>"DodecaHexa",13=>"Deca",_=>"Unsupported"};
}
