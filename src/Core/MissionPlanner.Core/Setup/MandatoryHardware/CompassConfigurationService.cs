...
public sealed class CompassConfigurationService : ICompassConfigurationService
{
    ...
    public bool WouldDisableOnlyEnabledCompass(CompassInventory inventory, int index)
    {
        ...
        return target is null || !target.Use ? false : inventory.Compasses.Count(compass => compass.Use) <= 1;
    }
    ...
    private async Task<CompassParameterApplyResult> ApplyAsync(VehicleId vehicleId, string parameterName, float value, string description, CancellationToken cancellationToken)
    {
        ...
        return await WriteAndConfirmAsync(vehicleId, parameterName, value, parameter.Type, cancellationToken).ConfigureAwait(false)
            ? new CompassParameterApplyResult(true, $"Confirmed {description} by vehicle readback.")
            : new CompassParameterApplyResult(false, $"Readback did not confirm the {description}. Reconnect, refresh, and verify before flying.");
    }
    ...
    {
        return state.Health.SensorsPresent is not { } present || (present & MagnetometerSensorBit) == 0 || state.Health.SensorsHealthy is not { } healthy
            ? null
            : (healthy & MagnetometerSensorBit) != 0;
    }
