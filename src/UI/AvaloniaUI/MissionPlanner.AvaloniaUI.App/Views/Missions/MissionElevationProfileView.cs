...
public sealed class MissionElevationProfileView : GraphicsView
{
    ...
    public MissionElevationProfile? Profile
    {
        get => (MissionElevationProfile?)GetValue(ProfileProperty); set => SetValue(ProfileProperty, value);
    }
    ...
    public MissionElevationProfileView()
    {
        Drawable = new ProfileDrawable();
    }

    private static void OnProfileChanged(BindableObject bindable, object oldValue, object newValue)
    {
        var view = (MissionElevationProfileView)bindable;
        ((ProfileDrawable)view.Drawable).Profile = (MissionElevationProfile?)newValue;
        view.Invalidate();
    }
    private sealed class ProfileDrawable : IDrawable
    {
        ...
        {
            canvas.FillColor = Color.FromArgb("#E6202024");
            canvas.FillRectangle(dirtyRect);
            if (Profile?.Samples.Count is not > 1)
            {
                return;
            }
            ...
            var width = Math.Max(1, dirtyRect.Width - 20);
            var height = Math.Max(1, dirtyRect.Height - 20);
        ...
        private static void DrawLine(ICanvas canvas, IEnumerable<(double Distance, double? Value)> values, Color color, double min, double range, double width, double height, double total)
        {
            ...
            foreach (var sample in values)
            {
                ...
                var x = (float)(10 + (sample.Distance / Math.Max(1, total) * width));
                var y = (float)(10 + height - ((sample.Value.Value - min) / range * height));
                if (path is null)
                {
                    path = new();
                    path.MoveTo(x, y);
