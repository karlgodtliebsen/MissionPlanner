...
public sealed class CubeLanDeviceAdapter : IVendorDeviceAdapter<CubeLanConfiguration>
{
    ...
    {
        ArgumentNullException.ThrowIfNull(configuration);
        var document = new { schema = configuration.Schema, deviceType = DeviceType, ports = configuration.Ports, vlanMembership = configuration.VlanMembership };
    ...
    {
        return expected.Ports.SequenceEqual(actual.Ports) && expected.VlanMembership.SequenceEqual(actual.VlanMembership);
    }
    ...
    {
        return new VendorDeviceApplyResult<CubeLanConfiguration>(false, message, snapshot, rollbackAttempted, rollbackSucceeded);
    }
