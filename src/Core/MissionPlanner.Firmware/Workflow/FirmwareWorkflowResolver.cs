...
public static class FirmwareWorkflowResolver
{
    ...
    public static FirmwareWorkflowCapabilities Resolve(FirmwareWorkflowContext context)
    {
        ...
        var automaticArduPilotEntry = serial && context.Runtime == FirmwareRuntimeKind.ArduPilot
            && context.RuntimeVerification == FirmwareRuntimeVerification.Verified
        ...
        var (code, reason) = context switch
        {
            ...
            _ when format == FirmwareArtifactFormat.Apj && !automaticArduPilotEntry && (context.Bootloader is null
            ...
            { TargetCompatible: false } when !automaticArduPilotEntry => ("target.compatibility-pending", "Identify the target and check firmware compatibility."),
