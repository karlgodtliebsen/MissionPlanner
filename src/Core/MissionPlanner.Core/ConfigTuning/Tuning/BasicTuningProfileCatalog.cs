...
public sealed class BasicTuningProfileCatalog : IBasicTuningProfileCatalog
{
    private static readonly IReadOnlyDictionary<FirmwareFamily, BasicTuningProfile> profiles =
        new Dictionary<FirmwareFamily, BasicTuningProfile> { [FirmwareFamily.ArduCopter] = Copter(), [FirmwareFamily.ArduPlane] = Plane(), [FirmwareFamily.Rover] = Rover(), [FirmwareFamily.ArduSub] = Sub() };

    /// <inheritdoc />
    public BasicTuningProfile? GetProfile(FirmwareFamily family)
    {
        return profiles.GetValueOrDefault(family);
    }

    private static BasicTuningProfile Copter()
    {
        return new BasicTuningProfile(
                ...
                Group(
                    ...
                    [
                        Field("input-time", "Input smoothing", "Time used to smooth pilot attitude inputs; lower values feel sharper.", "s", "ATC_INPUT_TC",
                            "Very low input smoothing can make the aircraft difficult to control."),
    ...
    private static BasicTuningProfile Plane()
    {
        return new BasicTuningProfile(
                ...
                Group(
                    ...
                    [
                        ...
                        Field("roll-limit", "Roll limit", "Maximum automatic bank angle.", "cdeg", "LIM_ROLL_CD",
                            "Increasing the roll limit raises stall and structural-load risk.")
                    ])
            ]);
    }

    private static BasicTuningProfile Rover()
    {
        return new BasicTuningProfile(
    ...
    private static BasicTuningProfile Sub()
    {
        return new BasicTuningProfile(
    ...
    private static BasicTuningGroupDefinition Group(
        ...
        string? warning = null)
    {
        return new BasicTuningGroupDefinition(key, title, description, fields, rules ?? [], warning);
    }
    ...
    private static BasicTuningFieldDefinition Field(
        ...
        string? warning = null)
    {
        return new BasicTuningFieldDefinition(key, title, description, units, ParameterFieldDefinition.Exact(parameterName), warning);
    }
    ...
    {
        return new BasicTuningFieldDefinition(key, title, description, units, new ParameterFieldDefinition(key, parameterNames));
    }
