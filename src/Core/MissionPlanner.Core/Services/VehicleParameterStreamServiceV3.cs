...
public sealed class VehicleParameterStreamServiceV3 : IVehicleParameterStreamService
{
        ...
        try
        {
            ...
            using var subscription = eventHub.SubscribeAsync<ParamValueMessage>(MavLinkEventTopics.NewMessage,
                async (message, ct) =>
                {
                    ...
                    // Store in registry
                    var parameter = new VehicleParameter(message.ParamId, message.ParamValue, message.ParamType, message.ParamIndex, message.ParamCount);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 0; attempt <= maxRetries; attempt++)
        {
            ...
            var result = await StreamAllParametersAsync(vehicleId, progress, null, TimeSpan.FromSeconds(30), cancellationToken);
