...
public interface IReplaySessionManager : IReplayClock, IAsyncDisposable
{
    ...
    ReplaySessionSnapshot Snapshot
    {
        get;
    }

    /// <summary>Occurs after replay state, clock, or projected vehicles change.</summary>
    event Action<ReplaySessionChangedEventArgs>? Changed;
    ...
    Task<ReplaySessionSnapshot> LoadAsync(Stream stream, string sourceName, CancellationToken cancellationToken = default);
    ...
    Task<ReplaySessionSnapshot> SeekAsync(TimeSpan position, CancellationToken cancellationToken = default);
