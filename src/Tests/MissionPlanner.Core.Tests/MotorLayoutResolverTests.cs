...
{
    [Theory]
    [InlineData(1,4)][InlineData(2,6)][InlineData(3,8)][InlineData(12,12)]
    ...
    [Fact]
    public void MissingFrameFailsClosed()=>new MotorLayoutResolver().Resolve(new Dictionary<string,VehicleParameter>()).Should().BeNull();
}
