...
public sealed class PayloadProtocolService(IVehicleComponentRegistry components, IVehicleRegistry vehicles,
    IVehicleConnectionSession connectionSession, IMavLinkCommandEncoder encoder, ICommandAckTracker acknowledgements,
    IVehicleOperationGate operationGate, IReplaySessionManager? replay = null) : ICameraProtocolService, IGimbalProtocolService
{
    private static readonly TimeSpan timeout = TimeSpan.FromSeconds(5);

    /// <inheritdoc />
    public IReadOnlyList<CameraComponentState> GetCameras(byte systemId) => components.GetComponents(systemId)
        .Where(item => item.MavType == (byte)MavType.Camera)
        .Select(item => new CameraComponentState(new(item.Key, "Camera", item.LastSeen), new(true, true, false, false))).ToArray();

    /// <inheritdoc />
    public IReadOnlyList<GimbalComponentState> GetGimbals(byte systemId) => components.GetComponents(systemId)
        .Where(item => item.MavType == (byte)MavType.Gimbal)
        .Select(item => new GimbalComponentState(new(item.Key, "Gimbal", item.LastSeen), new(true, true, false))).ToArray();
    ...
    public async Task<CameraOperationResult> CaptureImageAsync(VehicleId autopilot, byte componentId, CancellationToken cancellationToken)
    {
        ...
        return new(result.Accepted, result.Summary);
    }
        ...
        float yawDegrees, bool yawLock, CancellationToken cancellationToken)
    {
        if (!float.IsFinite(pitchDegrees) || pitchDegrees is < -90 or > 90 || !float.IsFinite(yawDegrees) || yawDegrees is < -180 or > 180)
            return new(false, "Pitch must be -90..90 and yaw -180..180 degrees.");
    ...
    {
        if (replay is not null && replay.Snapshot.State != ReplaySessionState.Unloaded) return (false, "Payload writes are blocked during replay.");
        var session = vehicles.GetRequired(autopilot);
        if (session is null) return (false, "The active vehicle is unavailable.");
        var target = new VehicleId(autopilot.SystemId, componentId);
        if (!operationGate.TryAcquire(autopilot, $"payload component {componentId}", out var lease)) return (false, "Another vehicle operation is active.");
        using (lease)
